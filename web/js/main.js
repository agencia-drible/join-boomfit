(function($) {
  ...
  function initContactForm() {
    var requiredInputs = $('#ajax-contact').find('input[data-required], textarea[data-required]').toArray();
    ...
    $(form).submit(function(e) {
          ...
          .done(function(response) {
                ...
                if(JSON.parse(response).status === 1){
                      ...
                      $('#email').val('');
                      $('#phone-number').val('');
                      $('#city-id').prop('selectedIndex',0);
                      $('#custom-data').prop('selectedIndex',0);
                }else{
    ...
    $('#ajax-contact input, #ajax-contact textarea').on('keyup', function(event) {
      event.preventDefault();
      if ($(this).val()) {
